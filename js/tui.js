...
function colorizeCode() {
  ...
  const keywords = {
    ...
    c: [
      ["NULL", "true", "false"],
      ["typedef", "struct", "void", "return", "if", "else", "while"],
    ],
    cpp: [
      ["NULL", "true", "false"],
      ["class", "public", "private", "void", "return", "if", "else", "while"],
    ],
    python: [
      ["None", "True", "False"],
...
async function displayContent() {
  try {
    ...
    console.log(`Loading content for section: ${sectionName}, item: ${currentPosition.sectionItemIndex}`);

    // Add loading indicator
    const loadingElement = document.createElement("div");
    loadingElement.innerHTML = '<p>Loading content...</p>';
    MAIN_CONTENT_SECTION.appendChild(loadingElement);

    // Fetch the data with absolute path to ensure it works regardless of route
    const response = await fetch(`/data/${sectionName}.json`);
    if (!response.ok) {
      throw new Error(`Failed to fetch ${sectionName}.json: ${response.status}`);
    }
    ...
    if (sectionName !== "home") {
      ...
      console.log(`Loaded data for ${sectionName} item: ${sectionData.name || currentPosition.sectionItemIndex}`);

      const topElement = document.createElement("div");

      // Add title if available (use name for projects and skills)
      const titleElement = document.createElement("h1");
      titleElement.innerHTML =
        sectionData.name != null
          ? `<span class="${getRandomTextColorClass()}">${sectionData.name}</span>`
          : null;
      ...
      logoElement.id = "logo";
      logoElement.alt = "Chaitany Patel";
    ...
    errorElement.innerHTML = `<p>Error loading content: ${error.message}</p>`;
    MAIN_CONTENT_SECTION.appendChild(errorElement);
  }
}
...
async function render(scrollToTop = false, isInitialRender = false) {
  try {
    console.log(`Rendering: section=${currentPosition.sectionIndex}, item=${currentPosition.sectionItemIndex}`);
    console.log(`Previous position: section=${previousPosition.sectionIndex}, item=${previousPosition.sectionItemIndex}`);
    ...
    if (
      ...
      console.log("No change in position, skipping render");
      return;
    }

    console.log(`Will render with new position: section=${currentPosition.sectionIndex}, item=${currentPosition.sectionItemIndex}`);
    ...
    if (currentSectionItemElement) {
      currentSectionItemElement.classList.add("selected-item");
      console.log(`Selected item: ${currentSectionItemElement.textContent}`);
    }
    ...
    // Handle scrolling for items not in view
...
function savePreviousPosition() {
  ...
  console.log(`Saved previous position: section=${previousPosition.sectionIndex}, item=${previousPosition.sectionItemIndex}`);
}

function goToSection(sectionNumber, itemNumber = null) {
  savePreviousPosition();

  // Save current item index if no specific item number is provided
  const currentItemIndex = itemNumber === null ? currentPosition.sectionItemIndex : itemNumber;
  ...
  // If we're moving to a different section and no specific item is requested,
  // try to maintain the same item index but make sure it's within bounds
  if (currentPosition.sectionIndex !== previousPosition.sectionIndex || itemNumber !== null) {
...
function goToNextSection() {
  ...
  const currentItemIndex = currentPosition.sectionItemIndex;
  ...
  // If we actually moved to a different section, try to maintain the same item index
  // but make sure it's within bounds of the new section's items
...
function goToNextItem() {
  ...
  // Log the current and new positions for debugging
  console.log(`goToNextItem: current=${currentPosition.sectionItemIndex}, max=${left_sections[currentPosition.sectionIndex].items.length - 1}`);
  ...
  console.log(`goToNextItem: new position=${currentPosition.sectionItemIndex}`);
}
...
function goToPreviousItem() {
  ...
  // Log the current and new positions for debugging
  console.log(`goToPreviousItem: current=${currentPosition.sectionItemIndex}, max=${left_sections[currentPosition.sectionIndex].items.length - 1}`);
  ...
  console.log(`goToPreviousItem: new position=${currentPosition.sectionItemIndex}`);
}
...
function initKeyboardListeners() {
  // Simple direct keyboard event listener
  document.addEventListener('keydown', async (event) => {
    ...
    try {
      if (key === "PageDown" || (ctrlKey && key === "d")) {
        ...
        // Save the current position before changing it
        const oldSectionIndex = currentPosition.sectionIndex;
        const oldItemIndex = currentPosition.sectionItemIndex;
        ...
        if (oldSectionIndex !== currentPosition.sectionIndex || oldItemIndex !== currentPosition.sectionItemIndex) {
          // Ensure we render after changing the item
          await render(false);
        }
        return; // Return early to avoid double rendering
        ...
        scrollToTop = true;
        console.log(`Jumping to section ${sectionNumber}`);
      } else {
...
function initMouseListeners() {
  try {
    console.log("Initializing mouse listeners...");

    // Add click handlers for each section
    left_sections.forEach((section, sectionIndex) => {
      console.log(`Setting up listeners for section: ${section.name}, items: ${section.items.length}`);

      // Add click handlers for each item in the section
      section.items.forEach((item, itemIndex) => {
        if (item) {
          ...
          item.addEventListener("click", async (event) => {
            event.stopPropagation();
            console.log(`Clicked on ${section.name} item ${itemIndex}`);
      ...
      // Add click handler for the section itself
      if (section.section) {
        section.section.addEventListener("click", async () => {
          console.log(`Clicked on section: ${section.name}`);
    ...
    console.error("Error initializing mouse listeners:", error);
  }
}
...
async function init() {
  try {
    ...
    if (!HOME_SECTION || !PROJECTS_SECTION || !SKILLS_SECTION) {
      console.error("Critical page sections not found:", {
        home: !!HOME_SECTION,
    ...
    console.log("All critical page sections found");

    // Make sure items are properly initialized
    if (PROJECTS_SECTION && PROJECTS_SECTION.firstElementChild && PROJECTS_SECTION.firstElementChild.children[2]) {
      left_sections[1].items = [...PROJECTS_SECTION.firstElementChild.children[2].children];
      console.log(`Projects items: ${left_sections[1].items.length}`);
    } else {
      console.error("Could not find projects items");
    }

    if (SKILLS_SECTION && SKILLS_SECTION.firstElementChild && SKILLS_SECTION.firstElementChild.children[2]) {
      left_sections[2].items = [...SKILLS_SECTION.firstElementChild.children[2].children];
      console.log(`Skills items: ${left_sections[2].items.length}`);
    } else {
      console.error("Could not find skills items");
    }

    // Make items look clickable
    left_sections.forEach(section => {
    ...
    console.log("Initializing touch listeners...");
    initTouchListeners();
...
window.goToPreviousSection = goToPreviousSection;
window.goToNextItem = goToNextItem;
window.goToPreviousItem = goToPreviousItem;
